...
```{r}
First_US_Cases <- filter(world_outbreak_data, iso_code=='USA') %>% select(Outbreak_start_date, Outbreak_end_date, outbreak_dates, level3_name, level2_name, level1_name, country_unique_code) %>% arrange(Outbreak_start_date) %>% distinct(level1_name, .keep_all = TRUE)
```
...
Potential_Source <- world_outbreak_data %>% filter( Outbreak_start_date > (Date_prior_infection-30) & Outbreak_start_date < Date_prior_infection-3)
```
...
Potential_Source$state_first_infection <- Date_prior_infection
```
...
state_connections <- function(state) {
  ...
  Date_prior_infection <- as_date(as.numeric(
    First_US_Cases[First_US_Cases$level1_name==state, 2]))
  Potential_Source <- world_outbreak_data %>% filter(
    Outbreak_start_date > (Date_prior_infection-30) &
      Outbreak_start_date < Date_prior_infection-3)
