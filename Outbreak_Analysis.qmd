...
### Create list of canidate sources
...
Then filter again to inlcude only locations that have connections to that state

```{r}
canidate_sources <- world_outbreak_data %>%
...
Add coloums to identify the state for each source & that state's first outbreak
```{r}
...
connection_finder <- function(state) {
  ...
  canidate_sources <<- rbind(canidate_sources, state_sources)
}
```
...
## Create summary of canidate source locations

```{r}
source_summary <- canidate_sources %>% group_by(state, state_first_infection, level3_name, level2_name, level1_name, country) %>% count %>% arrange(state_first_infection)
```
...
write.csv(canidate_sources,"Canidate_Source_List.csv", row.names = FALSE)
```
