...
Number of potential source countries and how many potential source events they had
```{r}
n_distinct(outside_US_sources$country)
sort(table(outside_US_sources$country))
```

Do the same for level1_names
...
## Time between event and infection
...
```{r}
source_summary <- candidate_sources %>% group_by(state, state_first_infection, country, level1_name) %>% count(level1_name)
```
