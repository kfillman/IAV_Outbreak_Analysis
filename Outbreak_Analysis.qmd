...
candidate_sources <- candidate_sources %>%
  select(state, state_first_infection, iso_code, country,
         region, sero_sub_genotype_eng, disease_eng, Outbreak_id, level3_name, level2_name, level1_name)
```
...
```
Do the same for level1_names (not inlcuding US)
```{r}
...
write.csv(First_US_Cases %>% select('State', 'County', 'Outbreak Start Date'),"US_First_Outbreaks.csv", row.names = FALSE)
```
