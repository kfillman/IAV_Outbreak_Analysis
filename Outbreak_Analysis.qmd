...
canidate_sources <- world_outbreak_data %>% filter((infectious_date %within% outbreak_dates) & (Outbreak_start_date < infectious_date))
```

### Match timely outbreaks with known state connections

```{r}
canidate_sources <- canidate_sources %>% filter(iso_code %in% state_connections$ISO.3166.1.alpha.3.x | iso_code %in% state_connections$ISO.3166.1.alpha.3.y)
canidate_sources$state <- 'South Carolina'
canidate_sources$state_first_infection <- infectious_date+3
```
...
connection_finder <- function(state) {
  ...
  canidate_sources <- world_outbreak_data %>% filter((infectious_date %within% outbreak_dates) & (Outbreak_start_date < infectious_date))
  canidate_sources <- canidate_sources %>% filter(
    iso_code %in% state_connections$ISO.3166.1.alpha.3.x |
      iso_code %in% state_connections$ISO.3166.1.alpha.3.y)
  canidate_sources$state <- state
  canidate_sources$state_first_infection <- as_date(as.numeric(
    First_US_Cases[First_US_Cases$level1_name==state, 1]))
  canidate_sources <<- rbind(canidate_sources, canidate_sources)
}
```
...
## Decrease variables in list of connections

```{r}
canidate_sources <- canidate_sources %>%
  select(state, state_first_infection, iso_code, country,
         region, sero_sub_genotype_eng, disease_eng, Outbreak_id, level3_name, 
         level2_name, level1_name)
```

## Create summary of introduction locations

```{r}
source_summary <- canidate_sources %>% group_by(state, state_first_infection, level3_name, level2_name, level1_name) %>% count %>% arrange(state_first_infection)
```

## Export

```{r}
write.csv(source_summary,"First_US_IAV_Outbreak_Overview.csv", row.names = FALSE)
write.csv(canidate_sources,"First_US_IAV_Outbreak_Sources.csv", row.names = FALSE)
```
