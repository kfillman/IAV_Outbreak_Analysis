...
First_US_Cases <- filter(world_outbreak_data, iso_code=='USA') %>% select(country_unique_code, level1_name, level2_name, Outbreak_start_date, outbreak_dates, reporting_level, strain_eng, sero_sub_genotype_eng,  disease_eng) %>% arrange(Outbreak_start_date) %>% distinct(level1_name, .keep_all = TRUE)
```
...
connection_finder <- function(state) {
  ...
  infectious_date <- as_date(as.numeric(
    First_US_Cases[First_US_Cases$level1_name==state, "Outbreak_start_date"])-3)
  state_sources <- world_outbreak_data %>%
  filter((infectious_date %within% outbreak_dates) & (Outbreak_start_date < infectious_date)) %>% filter(iso_code %in% state_connections$ISO.3166.1.alpha.3.x | iso_code %in% state_connections$ISO.3166.1.alpha.3.y)
...
candidate_sources <- candidate_sources %>%
  select(state, state_first_infection, iso_code, country, region, reporting_level, strain_eng, sero_sub_genotype_eng,  disease_eng, Outbreak_start_date, level3_name, level2_name, level1_name)
```
