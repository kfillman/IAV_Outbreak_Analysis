...
date_prior_infection is first day of state infection - minimum incubation period of infectious animal

```{r}
Date_prior_infection <- as_date(as.numeric (First_US_Cases [First_US_Cases$level1_name== 'South Carolina', 1])-3)
...
all_events <- Potential_Source %>%
  select(state, state_first_infection, iso_code, country,
         region, sero_sub_genotype_eng, level3_name, 
...
state_connections <- function(state) {
  ...
  Potential_Source$state_first_infection <- as_date(as.numeric(
...
```{r}
source_summary <- all_events %>% group_by(state, state_first_infection, level3_name, level2_name, level1_name) %>% count %>% arrange(state_first_infection)
```
